...
const typeCheckMap = {
    string: (value, options) => {
        ...
        if (options.max_length && value.length > options.max_length) {
            error = "Given string is longer than the specified maximum length";
        }
        if (options.min_length && value.length < options.min_length) {
            error = "Given string is shorter than the specified minimum length";
        }
        return error;
    },
    array: (value, options) => {
        // TODO Array options implementation
        let error;
        return error;
    },
